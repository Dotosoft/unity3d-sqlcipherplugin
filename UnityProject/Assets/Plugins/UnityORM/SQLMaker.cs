...
namespace UnityORM
{
	...
	{
		public readonly static DateTime UnixTime = new DateTime(1970,1,1, 0, 0, 0);
		
		public static string GenerateCreateTableSQL<T>(ClassDesc desc){
		...
		static string ConvertToSQLType(Type t){
			if(t == typeof(int) ||
				t == typeof(long)){
				return "INTEGER";
		...
		public static string GenerateSelectAllSQL<T>(ClassDesc desc){
			return "SELECT * FROM " + desc.Name + ";";
		}
		public static string GenerateSelectSQL<T>(ClassDesc desc,object key){
		...
		public static string GenerateDeleteAllSQL<T>(ClassDesc desc){
			return "DELETE FROM " + desc.Name + ";";
		}
		public static string GenerateDeleteSQL<T>(ClassDesc desc,object key){
			if(desc.KeyField == null) throw new Exception("Class " + desc.Name + " hasn't key field");
			return "DELETE FROM " + desc.Name + " WHERE " + desc.KeyField.NameInTable + " = " + ValueToBlock(key) + ";";
		}
		public static string GenerateInsertSQL<T>(ClassDesc desc,T obj){
			var builder = new StringBuilder();	
		...
		public static string GenerateUpdateSQL<T>(ClassDesc desc,T obj){
			if(desc.KeyField == null) throw new Exception("Class " + desc.Name + " hasn't key field");
			var builder = new StringBuilder();	
		...
		static string ValueToBlock(object v){
			if(v == null){
				return "NULL";
			}else{
